...
extern uint8_t slave_is_receiving;
	...
	if(PSCL!=0){
		switch(PDAT){
			case 0x00:
				...
				bitcnt=0;
				i2c_dat[0]=0;
				slave_is_receiving=1;
				MatchAddress=0;
				match=0;
			    EGN0 = 0x20;//FALLING_DATA;
			    EGP0 = 0x60;//RISING_BOTH;		
				break;
			case 0x10:
				i2c_state=2;
				if(slave_is_receiving==1)
					rcvlength=datcnt;			
				match=0;
...
static void __near r_intc6_interrupt(void)
{
	...
	if(PDAT==0x10 && slave_is_receiving==1 && bitcnt<8){
		 i2c_dat[datcnt]|=1<<(7-bitcnt);
	}
	else if(slave_is_receiving==0 && bitcnt<7){
	...
	switch(bitcnt){
		...
		switch(slave_is_receiving){
			case 1://if(slave_is_receiving==1){
			    EGN0 = 0x60;//FALLING_BOTH;
			    EGP0 = 0x20;//RISING_DATA;			
				break;//}
			case 0://else if(slave_is_receiving==0){
				P1  &=0b11101111;		
				NOP();
		...
		else if(slave_is_receiving==0 && PDAT==0x10){
		    EGN0 = 0x20;//FALLING_DATA;
		    EGP0 = 0x60;//RISING_BOTH;
				if(datcnt==9){
					i2c_state=2;
		...
		else if(slave_is_receiving==0 && PDAT==0x00){
		    EGN0 = 0x60;//FALLING_BOTH;
		    EGP0 = 0x20;//RISING_DATA;
				if(datcnt==9){
					i2c_state=2;
		...
		else if(slave_is_receiving==0 && EGP0 == 0x60 ){
			PM1 |=0b00010000;//SDA = input
		}
		else if(slave_is_receiving==0 && EGP0 == 0x20 ){
			PM1 &=0b11101111;	//SDA = output			
