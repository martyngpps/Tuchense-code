...
//	sprintf(test,"10%04lX%04lX%04lX%04lX%02X%02X%02X%02X%02X%02X%02X,%04lX,%0X\r\n",output[0],output[1],output[2],output[3],i2c_dat[1],i2c_dat[0],i2c_dat[2],i2c_dat[3],i2c_dat[4],i2c_dat[5],i2c_dat[6],coef[0][0],i2c_address);
//	sprintf(test,"%04X%04X%04X%04X\r\n",addresstest[0][0][0],addresstest[1][0][0],addresstest[0][0][1],addresstest[1][0][1]);
...
static void R_MAIN_UserInit(void)
{
	...
	R_INTC3_Start();// setup interrupts intc3 
	R_INTC5_Start();// setup interrupts intc5

  	POM1 = _10_POMn4_NCH_ON | _04_POMn2_NCH_ON ;// setting P1 Nch outputmode
 	PM1 = _80_PMn7_MODE_UNUSED | _40_PMn6_MODE_UNUSED | _20_PMn5_MODE_UNUSED | _10_PMn4_MODE_INPUT | 
          _00_PMn3_MODE_OUTPUT | _04_PMn2_MODE_INPUT  | _02_PMn1_MODE_UNUSED | _00_PMn0_MODE_OUTPUT;// setting P1 mode 	
...
/*	
	sprintf(test,"Read %d,%X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X\r\n",cnt,PFDLstatus,i2c_address
	,coefbuffer[ 0],coefbuffer[ 1],coefbuffer[ 2],coefbuffer[ 3]
	,coefbuffer[ 4],coefbuffer[ 5],coefbuffer[ 6],coefbuffer[ 7]
	,coefbuffer[ 8],coefbuffer[ 9],coefbuffer[10],coefbuffer[11]
	,coefbuffer[12],coefbuffer[13],coefbuffer[14],coefbuffer[15]
	,coefbuffer[16],coefbuffer[17],coefbuffer[18],coefbuffer[19]
	,coefbuffer[20],coefbuffer[21],coefbuffer[22],coefbuffer[23]
	);
	g_uart0_tx_end =R_UART0_Send((uint8_t *)test,strlen(test));
	while (g_uart0_tx_end==MD_OK) NOP();
	
	
	sprintf(test,"Coef %04lX %04lX %04lX %04lX %04lX %04lX %04lX %04lX %04lX %04lX %04lX %04lX\r\n"
	...
	for(setch=0;setch<4;setch++){
		...
		for(setcnt=4;setcnt>=0;setcnt--){
			...
			switch(setch){
				...
				case 1:PGA1CTL1=PGA1CTL1&0xE0;PGA1CTL1=PGA1CTL2|ofs_ctl1[setch];break;
				case 2:PGA2CTL1=PGA2CTL1&0xE0;PGA2CTL1=PGA2CTL2|ofs_ctl1[setch];break;
			}
